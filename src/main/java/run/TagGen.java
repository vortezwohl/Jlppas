...
public class TagGen {
    ...
    public static void main(String[] args) throws IOException, NoSuchAlgorithmException {
        ...
        Element sigma = G1.newOneElement();
        //生成U
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            sigma = sigma.duplicate().mul(xigemaivi);
            //生成μ
            mu = mu.duplicate().add(vi.duplicate().mulZn(mi));
            //生成1-s hmivi
        ...
        //验证
        Element t1 = pairing.pairing(sigma,g);
        System.out.println(t1);
        Element uMu = u.duplicate().powZn(mu);
        Element t2 = pairing.pairing(hmivi.duplicate().mul(uMu),g_x);
